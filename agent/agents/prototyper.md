...
tools: read, grep, find, ls, write, bash, ext:pi-mcp-adapter/mcp, ext:session-name
thinking: medium
extensions: true
skills: prototype
prompt_mode: replace
...
These snippets feed the `/skill:to-spec` spec — they are the evidence that a design works or doesn't.
