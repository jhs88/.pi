...
function resolveEditor(): { command: string; args: string[]; label: string } {
  const raw = process.env.EDITOR ?? "code";
  const parts = raw.split(/\s+/);
  return {
    command: parts[0],
    args: parts.slice(1),
    label: path.basename(parts[0]),
  };
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    if (changedFiles.size > 0) {
      ...
      ctx.ui.notify(
        `${changedFiles.size} changed file(s). Run /${commandName} to view/open in ${editor.label}.`,
        "info",
      );
    }
  });
  ...
  pi.registerCommand(commandName, {
    ...
    handler: async (args, ctx) => {
      ...
      const selected = await ctx.ui.select(
        `Open changed file in ${editor.label}`,
        labels,
      );
      if (!selected) return;
      ...
      if (result.code === 0) {
        ctx.ui.notify(`Opened ${selected} in ${editor.label}`, "info");
      } else {
        ctx.ui.notify(
          result.stderr.trim() ||
            `Failed to open ${selected} in ${editor.label}`,
          "error",
        );
      }
    },
