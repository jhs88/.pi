// Adapted from davis7dotsh/my-pi-setup@797eaf6d6f178759cf7aabde927ef15c91346e7e; hardened for session isolation.
/**
 ...
 *
 * Architecture: one dependency-free process supervisor per Pi session.
 * Node process and stream plumbing is bounded and callback-driven.
 */
...
import { createTerminalManager, type TerminalManager } from "./src/manager.ts";
import {
...
export default function (pi: ExtensionAPI) {
  ...
  const getManager = (): TerminalManager => {
    ...
    manager = createTerminalManager({
      spill: {
        root: path.join(getAgentDir(), "logs", "background-terminals"),
      },
    });
    manager.setOnSettled(onSettled);
    unsubStatus?.();
    unsubStatus = manager.subscribe(() => updateWidget(manager!));
  ...
  const deliverResults = (snapshots: readonly TerminalSnapshot[]) => {
    if (snapshots.length === 0) return true;
    const failed = snapshots.some((snap) => snap.status === "failed");
    const single = snapshots.length === 1 ? snapshots[0] : undefined;
    try {
      pi.sendMessage(
        {
          customType: "background-terminal-result",
          content: snapshots.map(buildTerminalResultMessage).join("\n\n---\n\n"),
          display: true,
          details: {
            id: single?.id ?? "batch",
            title: single?.title ?? `${snapshots.length} completions`,
            status: single?.status ?? (failed ? "failed" : "done"),
  ...
  const flushResults = () => {
    const snapshots = resultDelivery.drain();
    if (!deliverResults(snapshots)) {
      for (const snapshot of snapshots) resultDelivery.defer(snapshot);
    }
  };
  ...
  // transition: disposing the manager runs bounded SIGTERM→SIGKILL process-
  // tree cleanup. SIGKILL, hard crashes, and power loss cannot guarantee it.
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      const manager = getManager();
      const snap = manager.get(params.id);
      if (!snap) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal) {
      ...
      const report = await manager.kill(ids, signal);
  ...
  pi.registerCommand("ps", {
    ...
    handler: async (_args, ctx) => {
      ...
      await openTerminalPicker(ctx, manager);
    },
  });
}
